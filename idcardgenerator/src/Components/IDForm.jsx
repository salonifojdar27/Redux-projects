...
import { addCard, updateCard } from "../features/idcardSlice";

const IdForm = ({ onClose, editingCard }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingCard) {
      ...
      dispatch(addCard({ ...form, id: Date.now() }));
    }
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-gray-300 bg-opacity-50 flex justify-center items-center p-4">
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold">  {editingCard ? "Edit ID Card" : "Create ID Card"}</h2>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700" >{editingCard ? "Update" : "Add"}</button>
        </div>
      </form>
    </div>
  );
